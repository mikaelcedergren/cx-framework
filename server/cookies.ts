...
export function validateCookieValue(value: string): string {
  if (typeof value !== "string" || !COOKIE_VALUE_PATTERN.test(value)) {
    throw new Error(
      "Cookie values must use the RFC cookie-octet character set.",
    );
  }
  return value;
}
...
export function serializeCookie(
  ...
  if (
    ...
    throw new Error(
      "Cookie Max-Age must be a non-negative whole number of seconds.",
    );
  }
  if (
    expires !== undefined &&
  ...
  if (!(["lax", "none", "strict"] as const).includes(sameSite)) {
...
function validateCookiePath(path: string): string {
  if (
    typeof path !== "string" ||
    !path.startsWith("/") ||
    path.startsWith("//") ||
...
function capitalizeSameSite(value: "lax" | "none" | "strict"): string {
