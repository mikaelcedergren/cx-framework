...
export function sha256Hex(
  ...
  { maxInputBytes = MAX_HASH_INPUT_BYTES }: Sha256HexOptions = {},
): string {
  if (
    ...
    throw new Error(
      `SHA-256 maxInputBytes must be between 1 and ${MAX_HASH_INPUT_BYTES}.`,
    );
  }
  if (typeof value === "string") {
    ...
    if (Buffer.byteLength(value, "utf8") > maxInputBytes) {
      throw new Error(
        `SHA-256 input cannot exceed ${maxInputBytes} UTF-8 bytes.`,
      );
    }
...
export function randomBase64UrlIdentifier(
  byteLength = DEFAULT_RANDOM_IDENTIFIER_BYTES,
): string {
  if (
    ...
    throw new Error(
      `Random identifier byteLength must be between ${MIN_RANDOM_IDENTIFIER_BYTES} and ${MAX_RANDOM_IDENTIFIER_BYTES}.`,
    );
  }
...
export function createHmacTokenCodec({
  ...
  purpose,
}: HmacTokenCodecOptions): HmacTokenCodec {
  ...
  const activeSigningKey: KeyObject = activeKey;
  ...
  const maximumTokenCharacters =
    TOKEN_VERSION.length +
    1 +
  ...
  function signature(key: KeyObject, core: string): Buffer {
  ...
  function sign(payload: Uint8Array): string {
    ...
    if (payload.byteLength > safeMaxPayloadBytes) {
      throw new Error(
        `HMAC token payloads cannot exceed ${safeMaxPayloadBytes} bytes.`,
      );
    }
    const payloadBytes = Buffer.from(payload);
    const encodedPayload = payloadBytes.toString("base64url");
    const core = `${TOKEN_VERSION}.${safeActiveKeyId}.${encodedPayload}`;
    const encodedSignature = signature(activeSigningKey, core).toString(
      "base64url",
    );
    return `${core}.${encodedSignature}`;
  }

  function verify(token: string): VerifiedHmacToken | null {
    if (
      typeof token !== "string" ||
      token.length === 0 ||
  ...
  function signUtf8(payload: string): string {
  ...
  function verifyUtf8(token: string): VerifiedHmacTextToken | null {
...
function createKeyring(
  ...
  if (
    ...
    throw new Error(
      `An HMAC keyring must contain between 1 and ${MAX_KEY_COUNT} keys.`,
    );
  }
  ...
  for (const candidate of keys) {
    ...
    if (keyring.has(id)) throw new Error(`Duplicate HMAC key ID: ${id}.`);
...
function secretBytes(secret: HmacSecret): Buffer {
  let bytes: Buffer;
  if (typeof secret === "string") {
    assertWellFormedUtf16(secret, "HMAC string secrets");
    if (secret.length > MAX_SECRET_BYTES) {
      throw new Error(
        `HMAC secrets must contain between ${MIN_SECRET_BYTES} and ${MAX_SECRET_BYTES} bytes.`,
      );
    }
...
function validateKeyId(value: string): string {
  if (typeof value !== "string" || !KEY_ID_PATTERN.test(value)) {
    throw new Error(
      `HMAC key IDs must contain 1 to ${MAX_KEY_ID_CHARACTERS} URL-safe characters.`,
    );
  }
  return value;
}
...
function validatePurpose(value: string): string {
  if (typeof value !== "string" || !PURPOSE_PATTERN.test(value)) {
    throw new Error(
      "HMAC token purposes must contain 1 to 128 safe ASCII characters.",
    );
  }
  return value;
}

function validateMaxPayloadBytes(value: number): number {
  if (!Number.isSafeInteger(value) || value < 1 || value > MAX_PAYLOAD_BYTES) {
    throw new Error(
      `HMAC maxPayloadBytes must be between 1 and ${MAX_PAYLOAD_BYTES}.`,
    );
  }
  return value;
}
...
function base64UrlCharacterCount(byteCount: number): number {
