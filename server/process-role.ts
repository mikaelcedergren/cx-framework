...
export type ServerProcessRole =
  Readonly<{ kind: "web" }> | Readonly<{ kind: "worker"; key: string }>;

export type ValidatedServerProcessRole =
  | Readonly<{ kind: "web"; entrypoint: string }>
  | Readonly<{ kind: "worker"; key: string; entrypoint: string }>;
 ...
 */
export function assertServerProcessRole({
  artifactRoot,
  entrypointUrl,
  identity,
  role,
}: AssertServerProcessRoleOptions): ValidatedServerProcessRole {
  ...
  const declaredEntrypoint = requireRealFile(
    join(root, declaration.entrypoint),
    `Declared ${declaration.kind} entrypoint`,
  );
  assertContained(root, declaredEntrypoint, "Declared server entrypoint");

  if (actualEntrypoint !== declaredEntrypoint) {
    throw new Error(
      `The executing module does not match the sealed ${declaration.kind} entrypoint ${declaration.entrypoint}.`,
    );
  }
  return declaration;
}

function parseRole(value: ServerProcessRole): ServerProcessRole {
...
function workerDeclaration(
  ...
  key: string,
): Extract<ValidatedServerProcessRole, { kind: "worker" }> {
...
function requireEntrypointFile(value: string | URL): string {
  let url: URL;
  try {
...
function requireRealFile(value: string, label: string): string {
  const selected = realpathSync(value);
  const entry = statSync(selected);
