...
export interface SqliteMigrationLedgerAdoptionOptions extends SqliteMigrationOptions {
   ...
   */
  readonly verifyLegacyState: (
    database: SyncSqliteDatabase,
  ) => readonly VerifiedLegacySqliteMigration[];
}
...
interface PreparedMigrationLedgerAdoption {
  readonly appliedAt: string;
  readonly definition: PreparedMigration;
}
...
interface MigrationLedgerObjectRow extends SqliteRow {
 ...
 */
export function adoptSqliteMigrationLedger(
  database: SyncSqliteDatabase,
  migrations: readonly SqliteMigration[],
  options: SqliteMigrationLedgerAdoptionOptions,
): SqliteMigrationLedgerAdoptionResult {
  if (typeof options.fingerprint !== "function") {
    throw new Error(
      "SQLite migration-ledger adoption requires a fingerprint function.",
    );
  }
  if (typeof options.now !== "function") {
  ...
  if (typeof options.verifyLegacyState !== "function") {
    throw new Error(
      "SQLite migration-ledger adoption requires caller-side legacy state verification.",
    );
  }
  const prepared = prepareMigrations(migrations, options.fingerprint);
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (isPromiseLike(verifiedLegacyMigrations)) {
      throw new Error(
        "SQLite legacy state verification must be synchronous inside the adoption transaction.",
      );
    }
    const adoptions = prepareMigrationLedgerAdoptions(
      verifiedLegacyMigrations,
      prepared,
      options.now,
    );
    ...
    const adoptedVersions = adoptions.map(
      ({ definition }) => definition.migration.version,
    );
    return Object.freeze({
...
function prepareMigrationLedgerAdoptions(
  legacyMigrations: unknown,
  migrations: readonly PreparedMigration[],
  now: () => string,
): readonly PreparedMigrationLedgerAdoption[] {
  if (!Array.isArray(legacyMigrations) || legacyMigrations.length < 1) {
    throw new Error(
      "Verified legacy SQLite migrations must be a non-empty array.",
    );
  }
  ...
  return legacyMigrations.map((value, index) => {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
      throw new Error(
        `Verified legacy SQLite migration at index ${index} must be a record.`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Verified legacy SQLite migration at index ${index} must have a positive safe-integer version.`,
      );
    }
    ...
    if (!definition) {
      throw new Error(
        `Legacy SQLite ledger contains unknown migration version ${version}.`,
      );
    }
    const expectedVersion = index + 1;
    if (version !== expectedVersion) {
      throw new Error(
        `Legacy SQLite migration versions must be contiguous from 1; expected ${expectedVersion}, received ${version}.`,
      );
    }

    const appliedAtValue = legacy["appliedAt"];
    const appliedAt =
      appliedAtValue === undefined
        ? migrationTimestamp(now)
        : validateMigrationTimestamp(
            appliedAtValue,
            `Legacy SQLite migration ${version} appliedAt`,
          );
    return Object.freeze({ appliedAt, definition });
  });
}

function assertMigrationLedgerAbsent(database: SyncSqliteDatabase): void {
  const existing = database.get<MigrationLedgerObjectRow>(
  ...
  throw new Error(
    `Cannot adopt the SQLite migration ledger because ${schema}.${SQLITE_MIGRATION_LEDGER_TABLE} already exists as ${type}.`,
  );
}
