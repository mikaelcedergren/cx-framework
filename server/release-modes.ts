 ...
 */
export function assertRetainedBrowserReleaseModes(
  root: string,
  {
    maxDepth = DEFAULT_MAX_DEPTH,
    maxEntries = DEFAULT_MAX_ENTRIES,
  }: RetainedBrowserReleaseModeOptions = {},
): RetainedBrowserReleaseModeInventory {
  if (!Number.isSafeInteger(maxEntries) || maxEntries < 1) {
    throw new Error(
      "Retained browser release maxEntries must be a positive safe integer.",
    );
  }
  if (!Number.isSafeInteger(maxDepth) || maxDepth < 0) {
    throw new Error(
      "Retained browser release maxDepth must be a non-negative safe integer.",
    );
  }
  ...
  if (!rootSnapshot.isDirectory() || rootSnapshot.isSymbolicLink()) {
    throw new Error(
      `Retained browser release root must be a real directory: ${resolvedRoot}`,
    );
  }

  const records: ReleaseModeRecord[] = [];
  ...
  while (pending.length > 0) {
    ...
    if (current.depth > maxDepth) {
      throw new Error(
        `Retained browser release exceeds the ${maxDepth}-level depth ceiling: ${resolvedRoot}`,
      );
    }
    assertMode(
    ...
    try {
      for (
        ...
        if (snapshot.isSymbolicLink()) {
          throw new Error(
            `Retained browser release must not contain symbolic links: ${absolutePath}`,
          );
        }
        ...
        if (!snapshot.isFile() || snapshot.nlink !== 1n) {
          throw new Error(
            `Retained browser release contains a linked or unsupported entry: ${absolutePath}`,
          );
        }
        assertMode(
          snapshot.mode,
...
function assertMode(
  ...
  if (Number(actual & 0o7777n) !== expected) {
    throw new Error(
      `Retained browser release ${type} must have mode ${expected.toString(8).padStart(4, "0")}: ${absolutePath}`,
    );
  }
}

function assertEntryCeiling(
  entries: number,
  maximum: number,
  root: string,
): void {
  if (entries > maximum) {
    throw new Error(
      `Retained browser release exceeds the ${maximum}-entry ceiling: ${root}`,
    );
  }
}

function assertSnapshot(absolutePath: string, expected: BigIntStats): void {
  ...
  if (
    ...
    throw new Error(
      `Retained browser release entry changed during mode validation: ${absolutePath}`,
    );
  }
}
...
function requireSafePath(value: string): string {
  if (
    ...
    throw new Error(
      "Retained browser release root must be a safe non-empty path.",
    );
  }
  return value;
}
