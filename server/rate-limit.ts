...
export function createBoundedRateLimiter({
  ...
  function clock(): number {
    ...
    if (!Number.isSafeInteger(value) || value < 0) {
      throw new Error(
        "The rate-limit clock must return non-negative epoch milliseconds.",
      );
    }
    return value;
  }

  function pruneAt(timestamp: number): number {
    let removed = 0;
    for (const [key, entry] of entries) {
      if (timestamp < entry.resetAt) continue;
      entries.delete(key);
      removed += 1;
    }
    return removed;
    ...
    consume(key) {
      ...
      if (!entry) {
        ...
        if (!Number.isSafeInteger(resetAt)) {
          throw new Error(
            "The rate-limit reset time exceeds the safe integer range.",
          );
        }
  ...
  return (request, response, next) => {
    ...
    try {
      ...
      if (!Number.isSafeInteger(timestamp) || timestamp < 0) {
        throw new Error(
          "The rate-limit middleware clock must return non-negative epoch milliseconds.",
        );
      }
    } catch (error) {
      next(error);
    ...
    response.setHeader(
      "Retry-After",
      String(Math.max(1, Math.ceil((decision.resetAt - timestamp) / 1_000))),
    );
    next(new HttpError({ code, message, status: 429 }));
  };
}

function decision(value: RateLimitDecision): RateLimitDecision {
  return Object.freeze(value);
}

function earliestReset(entries: ReadonlyMap<string, RateLimitEntry>): number {
...
function validateRateLimitKey(key: string): string {
  if (
    typeof key !== "string" ||
    !key ||
    ...
    throw new Error(
      `Rate-limit keys must contain between 1 and ${MAX_RATE_LIMIT_KEY_LENGTH} characters.`,
    );
  }
  return key;
}
