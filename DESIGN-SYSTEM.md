...
- `ai/` contains portable AI design guidance and skills for downstream products.
...
Author or change an icon in `icons/svg/`, then validate and regenerate:

```sh
pnpm --filter @mikaelcedergren/cx-framework icons:check   # validate the contract (fails on any issue)
pnpm --filter @mikaelcedergren/cx-framework icons:index   # rebuild icons/index.html (visual QA)
pnpm --filter @mikaelcedergren/cx-framework icons         # rebuild icons/manifest.ts (cx-icon lookup)
```
