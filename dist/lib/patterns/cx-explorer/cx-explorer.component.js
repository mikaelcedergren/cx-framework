import { booleanAttribute, ChangeDetectionStrategy, Component, ElementRef, EventEmitter, Input, Output, ViewChild, computed, inject, signal, } from '@angular/core';
import { CdkDrag, CdkDragHandle, CdkDropList, moveItemInArray, } from '@angular/cdk/drag-drop';
import { cxIcons } from '../../icons/manifest.js';
...
export const CX_EXPLORER_DEFAULT_ITEM_ICONS = cxIcons.map((icon) => icon.name);
/**
...
export class CxExplorerComponent {
    ...
    scrollbarGutter = signal(0, /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "scrollbarGutter" }] : /* istanbul ignore next */ []));
    ...
    pickerPopover;
    pickerSearch = signal('', /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "pickerSearch" }] : /* istanbul ignore next */ []));
    ...
    pickerOverlay = new CxFloatingSurfaceController((rect) => ({
        width: rect.width,
    ...
    ...(ngDevMode ? [{ debugName: "filteredPickerIcons" }] : /* istanbul ignore next */ []));
    get resolvedWidth() {
        return this.resizedWidth() ?? this.width ?? '260px';
    }
    get resolvedMinWidth() {
    ...
    get renderedWidthPx() {
        return Math.round(this.explorerSurface?.nativeElement.getBoundingClientRect().width ?? EXPLORER_DEFAULT_WIDTH);
    }
    ...
    commitRename(event, current) {
        if (!this.isRenaming(current.id))
            return;
    ...
    focusPickerSearch() {
        this.pickerPopover?.surfaceElement()?.querySelector('input')?.focus();
    }
    ...
    static ɵcmp = i0.ɵɵngDeclareComponent({ minVersion: "17.0.0", version: "22.0.8", type: CxExplorerComponent, isStandalone: true, selector: "cx-explorer", inputs: { folders: "folders", rootItems: "rootItems", selectedItemId: "selectedItemId", loading: "loading", editable: ["editable", "editable", booleanAttribute], ariaLabel: "ariaLabel", width: "width", minWidth: "minWidth", resizable: ["resizable", "resizable", booleanAttribute], searchable: ["searchable", "searchable", booleanAttribute], searchValue: "searchValue", searchAriaLabel: "searchAriaLabel", persistenceKey: "persistenceKey", createItemText: "createItemText", itemIcons: "itemIcons", folderMenuItems: "folderMenuItems", itemMenuItems: "itemMenuItems" }, outputs: { selectedItemIdChange: "selectedItemIdChange", widthChange: "widthChange", searchValueChange: "searchValueChange", folderCreate: "folderCreate", itemCreate: "itemCreate", folderChange: "folderChange", folderOrderChange: "folderOrderChange", itemChange: "itemChange", folderDelete: "folderDelete", itemDelete: "itemDelete", menuAction: "menuAction" }, viewQueries: [{ propertyName: "explorerSurface", first: true, predicate: ["explorerSurface"], descendants: true, read: ElementRef }, { propertyName: "explorerContent", first: true, predicate: ["explorerContent"], descendants: true, read: ElementRef }, { propertyName: "pickerPopover", first: true, predicate: CxPopoverComponent, descendants: true }], ngImport: i0, template: "<aside\n  #explorerSurface\n  class=\"cx-explorer\"\n  [style.width]=\"resolvedWidth\"\n  [style.min-width]=\"resolvedMinWidth\"\n  [class.cx-explorer--resizable]=\"resizable\"\n  [attr.aria-label]=\"ariaLabel\"\n>\n  @if (resizable) {\n    <div\n      class=\"cx-explorer__resize-handle\"\n      [class.cx-explorer__resize-handle--active]=\"resizing()\"\n      role=\"separator\"\n      tabindex=\"0\"\n      aria-orientation=\"vertical\"\n      [attr.aria-label]=\"resolvedResizeAriaLabel\"\n      [attr.aria-valuenow]=\"renderedWidthPx\"\n      [attr.aria-valuetext]=\"renderedWidthPx + ' pixels'\"\n      (pointerdown)=\"onResizePointerDown($event)\"\n      (pointermove)=\"onResizePointerMove($event)\"\n      (pointerup)=\"onResizePointerUp($event)\"\n      (pointercancel)=\"onResizePointerCancel($event)\"\n      (lostpointercapture)=\"onResizeLostPointerCapture($event)\"\n      (keydown)=\"onResizeKeydown($event)\"\n      (dblclick)=\"onResizeDoubleClick($event)\"\n    ></div>\n  }\n\n  <div class=\"cx-explorer__header\">\n    <ng-content select=\"[header]\" />\n  </div>\n\n  @if (searchable) {\n    <cx-search-field\n      class=\"cx-explorer__search\"\n      [style.padding-inline-end.px]=\"scrollbarGutter()\"\n      label=\"\"\n      [ariaLabel]=\"searchAriaLabel\"\n      [clearable]=\"true\"\n      [value]=\"searchValue\"\n      (valueChange)=\"searchValueChange.emit($event)\"\n    />\n  }\n\n  @if (loading) {\n    <div class=\"cx-explorer__skeleton\" aria-hidden=\"true\">\n      @for (row of skeletonRows; track row) {\n        <span class=\"cx-explorer__skeleton-row\"></span>\n      }\n    </div>\n  } @else if (hasContent() || editable) {\n    <div #explorerContent class=\"cx-explorer__content\">\n      @if (rootItems$().length > 0) {\n        <ul class=\"cx-explorer__root-items\">\n          @for (item of rootItems$(); track item.id) {\n            <li class=\"cx-explorer__row cx-explorer__row--root\">\n              <button\n                class=\"cx-explorer__row-main\"\n                type=\"button\"\n                [class.cx-explorer__row-main--selected]=\"isSelected(item)\"\n                [attr.aria-current]=\"isSelected(item) ? 'true' : null\"\n                (click)=\"onItemPressed(item)\"\n              >\n                <span\n                  class=\"cx-explorer__row-icon cx-explorer__row-icon--accent\"\n                  [style.color]=\"itemAccent(item)\"\n                  aria-hidden=\"true\"\n                >\n                  <cx-icon [icon]=\"itemIcon(item)\" [size]=\"16\" />\n                </span>\n                <span class=\"cx-explorer__row-label cx-explorer__row-label--root\">{{\n                  item.name\n                }}</span>\n              </button>\n            </li>\n          }\n        </ul>\n      }\n\n      @if (hasFolders()) {\n        <div\n          class=\"cx-explorer__folders\"\n          cdkDropList\n          cdkDropListLockAxis=\"y\"\n          [cdkDropListData]=\"folders$()\"\n          (cdkDropListDropped)=\"onFolderDrop($event)\"\n        >\n          @for (folder of folders$(); track folder.id) {\n            <section\n              class=\"cx-explorer__folder\"\n              cdkDrag\n              [cdkDragData]=\"folder\"\n              [cdkDragDisabled]=\"!editable || isRenaming(folder.id)\"\n            >\n              <div class=\"cx-explorer__row cx-explorer__row--folder\" cdkDragHandle>\n                @if (isRenaming(folder.id)) {\n                  <span class=\"cx-explorer__row-icon\" aria-hidden=\"true\">\n                    <cx-icon icon=\"folder\" [size]=\"16\" />\n                  </span>\n                  <input\n                    class=\"cx-explorer__rename-input\"\n                    type=\"text\"\n                    aria-label=\"Folder name\"\n                    [value]=\"folder.name\"\n                    (blur)=\"commitRename($event, folder)\"\n                    (keydown.enter)=\"onRenameEnter($event)\"\n                    (keydown.escape)=\"cancelRename($event)\"\n                  />\n                } @else {\n                  <button\n                    class=\"cx-explorer__row-main\"\n                    type=\"button\"\n                    [attr.aria-expanded]=\"isFolderExpanded(folder)\"\n                    aria-keyshortcuts=\"Alt+ArrowUp Alt+ArrowDown\"\n                    (click)=\"toggleFolder(folder)\"\n                    (dblclick)=\"editable && beginRename(folder.id)\"\n                    (keydown.alt.arrowup)=\"onFolderMoveByKeyboard($event, folder, -1)\"\n                    (keydown.alt.arrowdown)=\"onFolderMoveByKeyboard($event, folder, 1)\"\n                  >\n                    <span class=\"cx-explorer__row-icon\" aria-hidden=\"true\">\n                      <cx-icon icon=\"folder\" [size]=\"16\" />\n                    </span>\n                    <span class=\"cx-explorer__row-label cx-explorer__row-label--folder\">{{\n                      folder.name\n                    }}</span>\n                    <cx-icon\n                      class=\"cx-explorer__chevron\"\n                      [icon]=\"isFolderExpanded(folder) ? 'chevron-up' : 'chevron-down'\"\n                      [size]=\"16\"\n                    />\n                  </button>\n                  @if (editable || hasFolderMenu()) {\n                    <div class=\"cx-explorer__row-actions\">\n                      @if (editable) {\n                        <cx-icon-button\n                          icon=\"plus\"\n                          size=\"small\"\n                          variant=\"transparent\"\n                          [ariaLabel]=\"createItemText\"\n                          [cxTooltip]=\"createItemText\"\n                          (pressed)=\"onCreateItem(folder)\"\n                        />\n                      }\n                      @if (hasFolderMenu()) {\n                        <cx-menu\n                          [presentation]=\"{ kind: 'trigger' }\"\n                          [items]=\"folderMenu()\"\n                          ariaLabel=\"Folder actions\"\n                          (itemSelect)=\"onFolderMenuSelect(folder, $event)\"\n                        >\n                          <cx-icon-button\n                            cxMenuTrigger\n                            icon=\"menu-vertical\"\n                            size=\"small\"\n                            variant=\"transparent\"\n                            ariaLabel=\"Folder actions\"\n                          />\n                        </cx-menu>\n                      }\n                    </div>\n                  }\n                }\n              </div>\n\n              @if (folder.items.length > 0) {\n                <div\n                  class=\"cx-explorer__items\"\n                  [class.cx-explorer__items--expanded]=\"isFolderExpanded(folder)\"\n                >\n                  <ul class=\"cx-explorer__items-inner\">\n                    @for (item of folder.items; track item.id) {\n                      <li class=\"cx-explorer__row cx-explorer__row--item\" #itemRow>\n                        <button\n                          class=\"cx-explorer__row-main\"\n                          type=\"button\"\n                          [class.cx-explorer__row-main--selected]=\"isSelected(item)\"\n                          [attr.aria-current]=\"isSelected(item) ? 'true' : null\"\n                          (click)=\"onItemPressed(item)\"\n                        >\n                          <span\n                            class=\"cx-explorer__row-icon cx-explorer__row-icon--accent\"\n                            [style.color]=\"itemAccent(item)\"\n                            aria-hidden=\"true\"\n                          >\n                            <cx-icon [icon]=\"itemIcon(item)\" [size]=\"16\" />\n                          </span>\n                          <span class=\"cx-explorer__row-label\">{{ item.name }}</span>\n                        </button>\n                        <div class=\"cx-explorer__row-actions\">\n                          @if (hasItemMenu()) {\n                            <cx-menu\n                              [presentation]=\"{ kind: 'trigger' }\"\n                              [items]=\"itemMenu()\"\n                              ariaLabel=\"Page actions\"\n                              (itemSelect)=\"onItemMenuSelect(item, $event, itemRow)\"\n                            >\n                              <cx-icon-button\n                                cxMenuTrigger\n                                icon=\"menu-vertical\"\n                                size=\"small\"\n                                variant=\"transparent\"\n                                ariaLabel=\"Page actions\"\n                              />\n                            </cx-menu>\n                          }\n                        </div>\n                      </li>\n                    }\n                  </ul>\n                </div>\n              }\n            </section>\n          }\n        </div>\n      }\n      @if (editable) {\n        <cx-icon-button\n          class=\"cx-explorer__new-folder\"\n          icon=\"plus\"\n          ariaLabel=\"New folder\"\n          cxTooltip=\"New folder\"\n          [block]=\"true\"\n          (pressed)=\"onFolderCreate()\"\n        />\n      }\n    </div>\n  }\n</aside>\n\n@if (pickerItem(); as item) {\n  <cx-popover\n    [open]=\"true\"\n    [showBackdrop]=\"true\"\n    [width]=\"pickerOverlay.width$()\"\n    [left]=\"pickerOverlay.left$()\"\n    [top]=\"pickerOverlay.top$()\"\n    [bottom]=\"pickerOverlay.bottom$()\"\n    [maxHeight]=\"pickerOverlay.maxHeight$()\"\n    [placement]=\"pickerOverlay.placement$()\"\n    role=\"dialog\"\n    heading=\"Icon & color\"\n    ariaLabel=\"Icon & color\"\n    surfaceVariant=\"grouped\"\n    (backdropPressed)=\"closePicker()\"\n  >\n    <cx-button\n      actions\n      text=\"Reset\"\n      size=\"small\"\n      [transparent]=\"true\"\n      [disabled]=\"!item.icon && !item.color\"\n      (pressed)=\"onPickerReset(item)\"\n    />\n    <div class=\"cx-explorer__picker\">\n      <div class=\"cx-explorer__picker-group\">\n        <div class=\"cx-explorer__picker-swatches\" role=\"group\" aria-label=\"Color\">\n          <button\n            class=\"cx-explorer__swatch cx-explorer__swatch--none\"\n            type=\"button\"\n            aria-label=\"No color\"\n            [attr.aria-pressed]=\"!item.color\"\n            (click)=\"onPickerColor(item, undefined)\"\n          ></button>\n          @for (color of pickerColors; track color) {\n            <button\n              class=\"cx-explorer__swatch\"\n              type=\"button\"\n              [style.background]=\"\n                'color-mix(in srgb, ' + swatchAccent(color) + ' 80%, var(--surface))'\n              \"\n              [attr.aria-label]=\"color\"\n              [attr.aria-pressed]=\"item.color === color\"\n              (click)=\"onPickerColor(item, color)\"\n            ></button>\n          }\n        </div>\n      </div>\n      <div class=\"cx-explorer__picker-group\">\n        <cx-search-field\n          class=\"cx-explorer__picker-search\"\n          label=\"\"\n          ariaLabel=\"Search icons\"\n          [clearable]=\"true\"\n          [value]=\"pickerSearch()\"\n          (valueChange)=\"pickerSearch.set($event)\"\n        />\n        <div class=\"cx-explorer__picker-icons\" role=\"group\" aria-label=\"Icon\">\n          @for (icon of filteredPickerIcons(); track icon) {\n            <button\n              class=\"cx-explorer__picker-icon\"\n              type=\"button\"\n              [attr.aria-label]=\"icon\"\n              [attr.aria-pressed]=\"isPickerIconSelected(item, icon)\"\n              [style.color]=\"isPickerIconSelected(item, icon) ? itemAccent(item) : null\"\n              (click)=\"onPickerIcon(item, icon)\"\n            >\n              <cx-icon [icon]=\"icon\" [size]=\"16\" />\n            </button>\n          } @empty {\n            <p class=\"cx-explorer__picker-empty\" role=\"status\">No icons found</p>\n          }\n        </div>\n      </div>\n    </div>\n  </cx-popover>\n}\n", styles: [":host{display:block;height:100%;min-height:0}.cx-explorer{display:flex;position:relative;width:260px;min-width:min(200px,100%);max-width:100%;height:100%;min-height:100%;flex-direction:column;gap:var(--space-md);padding:var(--space-sm);border-right:var(--line);background:var(--surface);box-sizing:border-box;overflow:visible}.cx-explorer__header{display:flex;width:100%;height:var(--controller-size);min-height:var(--controller-size);align-items:center;flex:0 0 auto}.cx-explorer__new-folder{margin-top:var(--space-sm);flex:0 0 auto;opacity:0;transition:opacity var(--motion-fast) ease}.cx-explorer:hover .cx-explorer__new-folder,.cx-explorer__new-folder:focus-within{opacity:1}@media(hover: none){.cx-explorer__new-folder{opacity:1}}.cx-explorer__resize-handle{position:absolute;top:0;inset-inline-end:calc(-1*(var(--space-xs) + var(--border-width)));z-index:2;width:12px;height:100%;cursor:col-resize;touch-action:none}.cx-explorer__resize-handle::after{position:absolute;top:50%;inset-inline-end:var(--space-xs);width:3px;height:calc(100% - var(--space-sm)*2);border-radius:var(--radius-pill);corner-shape:round;background:rgba(0,0,0,0);content:\"\";transform:translateY(-50%);transition:background-color var(--motion-fast) ease}.cx-explorer__resize-handle:hover::after,.cx-explorer__resize-handle:focus-visible::after,.cx-explorer__resize-handle--active::after{background:var(--info)}.cx-explorer__resize-handle:focus-visible{outline:var(--outline-tab);outline-offset:calc(-1*var(--outline-tab-offset))}.cx-explorer__search{box-sizing:border-box;width:100%;flex:0 0 auto;padding-inline-end:0px}.cx-explorer__skeleton{display:flex;min-height:0;flex:1 1 auto;width:100%;flex-direction:column;gap:var(--space-xs);padding:var(--space-xs) 0}.cx-explorer__skeleton-row{display:block;height:calc(var(--controller-size-small) + var(--space-sm)*2);border-radius:var(--radius-sm);background:linear-gradient(90deg, var(--opacity-low), var(--opacity-mid), var(--opacity-low));background-size:180% 100%;animation:cx-explorer-skeleton 1.2s ease-in-out infinite}.cx-explorer__content{display:flex;min-height:0;flex:1 1 auto;width:100%;flex-direction:column;gap:0;overflow:auto;overscroll-behavior:contain}.cx-explorer__root-items,.cx-explorer__folders{display:flex;width:100%;margin:0;padding:0;flex-direction:column}.cx-explorer__root-items{gap:var(--space-2xs);list-style:none}.cx-explorer__folders{gap:0}.cx-explorer__folder{display:flex;width:100%;flex-direction:column;gap:var(--space-2xs)}.cx-explorer__folder.cdk-drag-preview{border-radius:var(--radius-md);background:var(--surface-alt);box-shadow:var(--shadow-mid)}.cx-explorer__folder.cdk-drag-preview .cx-explorer__items{display:none}.cx-explorer__folder.cdk-drag-placeholder{opacity:0}.cx-explorer__folder.cdk-drag-animating,.cx-explorer__folders.cdk-drop-list-dragging .cx-explorer__folder:not(.cdk-drag-placeholder){transition:transform var(--motion-base) var(--ease-out)}.cx-explorer__row{position:relative;display:flex;width:100%;min-width:0;align-items:center;border-radius:var(--radius-md)}.cx-explorer__row:hover{background:var(--opacity-mid)}.cx-explorer__row-main{display:flex;min-width:0;min-height:calc(var(--controller-size-small) + var(--space-sm)*2);flex:1 1 auto;align-items:center;gap:var(--space-sm);padding:var(--space-sm) var(--space-xs);border:0;border-radius:var(--radius-md);background:rgba(0,0,0,0);color:var(--opacity-high);font:inherit;text-align:left;cursor:pointer}.cx-explorer__row:hover .cx-explorer__row-main{color:var(--ink)}.cx-explorer__row-main:focus-visible{outline:var(--outline-tab);outline-offset:var(--outline-tab-offset)}.cx-explorer__row-main--selected{color:var(--ink);font-weight:var(--font-weight-bold)}.cx-explorer__row--root,.cx-explorer__row--item{list-style:none}.cx-explorer__row--item .cx-explorer__row-main{min-height:calc(var(--controller-size-small) + var(--space-xs)*2);padding-block:var(--space-xs);padding-inline-start:var(--space-xl)}.cx-explorer__row-icon{display:inline-flex;width:var(--controller-size-small);height:var(--controller-size-small);flex:0 0 var(--controller-size-small);align-items:center;justify-content:center}.cx-explorer__row-icon--accent{color:inherit}.cx-explorer__row--folder .cx-explorer__row-icon{color:var(--purple)}.cx-explorer__row-label{min-width:0;flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:var(--font-size-body);line-height:1.2}.cx-explorer__row-label--folder{font-weight:var(--font-weight-medium)}.cx-explorer__row-label--root{font-weight:var(--font-weight-medium)}.cx-explorer__chevron{flex:0 0 auto;color:var(--opacity-high)}.cx-explorer__row--folder .cx-explorer__row-main{padding-inline-end:calc(var(--space-xs) + var(--controller-size-small))}.cx-explorer__row--folder:has(.cx-explorer__row-actions) .cx-explorer__row-main{padding-inline-end:calc(var(--space-xs) + var(--controller-size-small) + var(--controller-size-small) + var(--space-2xs))}.cx-explorer__row--folder:has(.cx-explorer__row-actions>cx-icon-button) .cx-explorer__row-main{padding-inline-end:calc(var(--space-xs) + var(--controller-size-small) + var(--controller-size-small) + var(--controller-size-small) + var(--space-2xs) + var(--space-2xs))}.cx-explorer__row--folder .cx-explorer__chevron{position:absolute;inset-inline-end:var(--space-xs);display:inline-flex;width:var(--controller-size-small);height:var(--controller-size-small);align-items:center;justify-content:center;pointer-events:none}.cx-explorer__row--folder .cx-explorer__row-actions{position:absolute;inset-inline-end:calc(var(--space-xs) + var(--controller-size-small) + var(--space-2xs));z-index:1;padding-right:0}.cx-explorer__row-actions{display:flex;flex:0 0 auto;align-items:center;gap:var(--space-2xs);padding-right:var(--space-2xs);opacity:0;transition:opacity var(--motion-fast) var(--ease-out-in)}.cx-explorer__row:hover .cx-explorer__row-actions,.cx-explorer__row-actions:focus-within,.cx-explorer__row-actions:has([aria-expanded=true]){opacity:1}@media(hover: none){.cx-explorer__row-actions{opacity:1}}.cx-explorer__rename-input{min-width:0;min-height:calc(var(--controller-size-small) + var(--space-xs)*2);flex:1 1 auto;margin:0;padding:var(--space-xs);border:var(--border-width) solid var(--primary);border-radius:var(--radius-sm);background:var(--surface);color:var(--ink);font:inherit;font-size:var(--font-size-body);line-height:1.2}.cx-explorer__rename-input:focus-visible{outline:none}.cx-explorer__items{display:grid;width:100%;grid-template-rows:0fr;transition:grid-template-rows var(--motion-slow) var(--ease-out-strong)}.cx-explorer__items--expanded{grid-template-rows:1fr}.cx-explorer__items-inner{display:flex;min-height:0;margin:0;padding:0 0 0 0;flex-direction:column;gap:var(--space-2xs);list-style:none;overflow:hidden;visibility:hidden;pointer-events:none;opacity:0;transform:translateY(-6px);transition:opacity var(--motion-base) ease,transform var(--motion-slow) var(--ease-out-strong)}.cx-explorer__items--expanded .cx-explorer__items-inner{visibility:visible;pointer-events:auto;opacity:1;transform:translateY(0)}.cx-explorer__picker{display:flex;min-width:0;min-height:0;flex-direction:column;gap:var(--surface-separation)}.cx-explorer__picker-group{display:flex;min-width:0;flex:0 0 auto;flex-direction:column;gap:var(--space-md);padding:var(--space-md);overflow:hidden;border-radius:var(--radius-md);background:var(--surface)}.cx-explorer__picker-swatches{display:grid;flex-shrink:0;grid-template-columns:repeat(6, minmax(0, var(--controller-size)));gap:var(--space-sm)}.cx-explorer__swatch{width:100%;aspect-ratio:1;padding:0;border:var(--border-width) solid rgba(0,0,0,0);border-radius:var(--radius-sm);cursor:pointer}.cx-explorer__swatch--none{background:color-mix(in srgb, var(--ink) 30%, var(--surface))}.cx-explorer__swatch:hover{border-color:var(--opacity-mid)}.cx-explorer__swatch[aria-pressed=true]{border-color:var(--ink);box-shadow:inset 0 0 0 2px var(--surface)}.cx-explorer__swatch:focus-visible{outline:var(--outline-tab);outline-offset:var(--outline-tab-offset)}.cx-explorer__picker-search,.cx-explorer__picker-icons{min-width:0;contain:inline-size}.cx-explorer__picker-icons{display:grid;grid-template-columns:repeat(6, minmax(0, 1fr));gap:var(--space-sm);max-height:calc(var(--controller-size-small)*6 + var(--space-sm)*5);overflow-y:auto;overscroll-behavior:contain}.cx-explorer__picker-empty{grid-column:1/-1;margin:0;color:var(--opacity-high);font-size:var(--font-size-body)}.cx-explorer__picker-icon{display:inline-flex;min-height:var(--controller-size-small);align-items:center;justify-content:center;padding:var(--space-2xs);border:0;border-radius:var(--radius-sm);background:rgba(0,0,0,0);color:var(--opacity-high);cursor:pointer}.cx-explorer__picker-icon:hover{background:var(--opacity-mid);color:var(--ink)}.cx-explorer__picker-icon[aria-pressed=true]{background:var(--opacity-low);color:inherit}.cx-explorer__picker-icon:focus-visible{outline:var(--outline-tab);outline-offset:var(--outline-tab-offset)}@media(prefers-reduced-motion: reduce){.cx-explorer__items,.cx-explorer__items-inner,.cx-explorer__skeleton-row,.cx-explorer__row-actions,.cx-explorer__new-folder{transition:none;animation:none}}@keyframes cx-explorer-skeleton{0%{background-position:100% 0}100%{background-position:-80% 0}}"], dependencies: [{ kind: "directive", type: CdkDrag, selector: "[cdkDrag]", inputs: ["cdkDragData", "cdkDragLockAxis", "cdkDragRootElement", "cdkDragBoundary", "cdkDragStartDelay", "cdkDragFreeDragPosition", "cdkDragDisabled", "cdkDragConstrainPosition", "cdkDragPreviewClass", "cdkDragPreviewContainer", "cdkDragScale"], outputs: ["cdkDragStarted", "cdkDragReleased", "cdkDragEnded", "cdkDragEntered", "cdkDragExited", "cdkDragDropped", "cdkDragMoved"], exportAs: ["cdkDrag"] }, { kind: "directive", type: CdkDragHandle, selector: "[cdkDragHandle]", inputs: ["cdkDragHandleDisabled"] }, { kind: "directive", type: CdkDropList, selector: "[cdkDropList], cdk-drop-list", inputs: ["cdkDropListConnectedTo", "cdkDropListData", "cdkDropListOrientation", "id", "cdkDropListLockAxis", "cdkDropListDisabled", "cdkDropListSortingDisabled", "cdkDropListEnterPredicate", "cdkDropListSortPredicate", "cdkDropListAutoScrollDisabled", "cdkDropListAutoScrollStep", "cdkDropListElementContainer", "cdkDropListHasAnchor"], outputs: ["cdkDropListDropped", "cdkDropListEntered", "cdkDropListExited", "cdkDropListSorted"], exportAs: ["cdkDropList"] }, { kind: "component", type: CxButtonComponent, selector: "cx-button", inputs: ["text", "mood", "icon", "appendIcon", "shortcutParts", "href", "type", "size", "ariaLabel", "disabled", "transparent", "rounded", "loading"], outputs: ["pressed"] }, { kind: "component", type: CxIconButtonComponent, selector: "cx-icon-button", inputs: ["icon", "ariaLabel", "role", "ariaHasPopup", "ariaExpanded", "ariaControls", "mood", "variant", "size", "selected", "ariaPressed", "rounded", "disabled", "badgeValue", "block", "loading", "countdown"], outputs: ["pressed", "countdownChange"] }, { kind: "component", type: CxIconComponent, selector: "cx-icon", inputs: ["icon", "size", "mood", "shape"] }, { kind: "component", type: CxSearchFieldComponent, selector: "cx-search-field", inputs: ["label", "ariaLabel", "hint", "optional", "disabled", "loading", "clearable", "size", "validation", "value"], outputs: ["valueChange", "focusChange"] }, { kind: "component", type: CxMenuComponent, selector: "cx-menu", inputs: ["disabled", "presentation", "ariaLabel", "heading", "items", "groups", "currentId", "shortcutsEnabled", "open", "align", "placement", "layout", "width"], outputs: ["openChange", "itemSelect", "currentIdChange"] }, { kind: "directive", type: CxMenuTriggerDirective, selector: "[cxMenuTrigger]" }, { kind: "component", type: CxPopoverComponent, selector: "cx-popover", inputs: ["open", "showBackdrop", "owner", "surfaceId", "role", "ariaLabel", "heading", "left", "top", "bottom", "width", "minWidth", "maxWidth", "maxHeight", "placement", "surfaceVariant"], outputs: ["backdropPressed"] }, { kind: "directive", type: CxTooltipDirective, selector: "[cxTooltip]", inputs: ["cxTooltip", "cxTooltipPosition", "cxTooltipDelay", "cxTooltipDisabled", "cxTooltipOverflow"] }], changeDetection: i0.ChangeDetectionStrategy.OnPush });
}
