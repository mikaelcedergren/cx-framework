...
function normalizeCxImage(image) {
    for (const value of [image?.intrinsicWidth, image?.intrinsicHeight]) {
        if (value !== undefined && (!Number.isInteger(value) || value <= 0)) {
            throw new Error("cx-image intrinsic dimensions must be positive integers.");
        }
    }
    return {
        src: image?.src ?? "",
...
function calculateImageStyles(image) {
    return {
        ...
        "max-width": image.maxWidth === "auto"
            ? "none"
            : resolveImageSizeValue(image.maxWidth),
        "max-height": image.maxHeight === "auto"
            ? "none"
