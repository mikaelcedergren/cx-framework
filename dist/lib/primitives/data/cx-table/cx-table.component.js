import { CxTableCellDirective } from './cx-table-cell.directive.js';
import { CommonModule } from '@angular/common';
import { ChangeDetectionStrategy, Component, EventEmitter, HostListener, Injector, Input, Output, ViewChild, afterNextRender, afterRenderEffect, computed, contentChildren, inject, signal, } from '@angular/core';
...
function hasSerializableFilterValue(value) {
    ...
    const span = value;
    return Boolean(span.start || span.end || typeof span.min === 'number' || typeof span.max === 'number');
}
export class CxTableComponent {
    ...
    set rows(value) {
        ...
        if (this.pendingContextMenuState &&
    ...
    onRowMenuOpenChange(rowId, open) {
        if (open) {
            if (this.rowsState().find((row) => row.id === rowId)?.disabled)
                return;
            this.closeContextMenu(false);
            this.closeColumnHeaderMenu(false);
    ...
    closeContextMenu(restoreFocus = true) {
        ...
        if (restoreFocus &&
            ...
            queueMicrotask(() => {
                ...
                if (row && this.rowIsKeyboardReachable(row))
                    contextMenu.originRow?.focus();
            });
        }
    }
    isRowSelected(rowId) {
    ...
    toggleAllRowsSelection(checked) {
        ...
        const preservedIds = this.selectedRowIdsState().filter((rowId) => !selectableRowIdSet.has(rowId));
    ...
    cellTemplates = contentChildren(CxTableCellDirective, { ...(ngDevMode ? { debugName: "cellTemplates" } : /* istanbul ignore next */ {}), descendants: true });
    customCellTemplate(columnId) {
    ...
    focusColumnHeaderMenuWhenReady(column, attempt = 0) {
        ...
        const firstAction = surface?.querySelector('.cx-table__header-menu-actions button:not(:disabled)');
        if (firstAction) {
            firstAction.focus();
            return;
        }
    ...
    syncColumnHeaderMenuPosition(column, triggerElement) {
        ...
        const propertyActionCount = Number(this.isColumnPinnable(column)) + Number(this.isColumnHideable(column));
    ...
    applyKeyColumnAutoFit() {
        ...
        const autoFitOwnsWidth = override === undefined || (applied?.columnId === keyColumn.id && applied.width === override);
        if (!autoFitOwnsWidth) {
            return true;
        }
    ...
    commitKeyboardColumnReorder() {
        ...
        this.draggingColumnIdState.set(undefined);
        this.columnReorderAnnouncementState.set(changed
    ...
    eventComesFromInteractiveDescendant(event, rowElement) {
        ...
        const path = composedPath.length > 0 ? composedPath : this.elementPathToRow(event.target, rowElement);
...
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "22.0.8", ngImport: i0, type: CxTableComponent, decorators: [{
            ...
            args: [{ selector: 'cx-table', imports: [
                ...
                type: Output
            }], cellTemplates: [{ type: i0.ContentChildren, args: [i0.forwardRef(() => CxTableCellDirective), { ...{ descendants: true }, isSignal: true }] }], onWindowResize: [{
                type: HostListener,
                args: ['window:resize']
            }] } });
