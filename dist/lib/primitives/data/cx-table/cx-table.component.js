...
export class CxTableComponent {
    ...
    appliedFirstColumnAutoFit;
    firstColumnAutoFitTimer;
    ...
    noMatchesText = 'No results match the current filters.';
    ...
    skeletonRows = Array.from({ length: 5 }, (_, index) => index);
    ...
    rowIds$ = computed(() => this.rowsState().map(row => row.id), /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "rowIds$" }] : /* istanbul ignore next */ []));
    ...
    applyFirstColumnAutoFit() {
        ...
        const width = this.capFirstColumnWidthToViewport(firstColumn.id, this.clampColumnWidth(this.autoFitColumnWidth(firstColumn.id)));
     ...
     */
    capFirstColumnWidthToViewport(columnId, width) {
