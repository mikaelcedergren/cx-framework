...
export function renderCxQrCodeSvg(data, errorCorrection, pixelSize = 192) {
...
function pickVersion(byteLength, errorCorrection) {
...
function encodeDataCodewords(bytes, version, errorCorrection) {
    const capacityBits = getNumDataCodewords(version, errorCorrection) * 8;
    const characterCountBits = version <= 9 ? 8 : 16;
...
function addErrorCorrectionAndInterleave(dataCodewords, version, errorCorrection) {
...
function drawAlignmentPatterns(modules, functionModules, version) {
    const positions = getAlignmentPatternPositions(version);
    const last = positions.length - 1;
    positions.forEach((x, xIndex) => {
        positions.forEach((y, yIndex) => {
            const overlapsFinder = (xIndex === 0 && yIndex === 0) || (xIndex === 0 && yIndex === last) || (xIndex === last && yIndex === 0);
            if (!overlapsFinder) {
...
function drawFormatBits(modules, functionModules, version, errorCorrection, mask) {
    const size = version * 4 + 17;
...
function drawCodewords(modules, functionModules, codewords) {
    const bits = codewords.flatMap(codeword => Array.from({ length: 8 }, (_, index) => ((codeword >>> (7 - index)) & 1) !== 0));
    let bitIndex = 0;
    let upward = true;
...
function applyBestMask(modules, functionModules, version, errorCorrection) {
...
function getLineRunPenalty(line) {
    let penalty = 0;
    let runColor = line[0];
    let runLength = 1;
...
function getFinderLikePenalty(line) {
    let penalty = 0;
    const patterns = [
...
function getNumDataCodewords(version, errorCorrection) {
    const rawCodewordCount = getNumRawDataModules(version) / 8;
    return (rawCodewordCount -
...
function toSvg(modules, pixelSize) {
    const size = modules.length;
