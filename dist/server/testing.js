 ...
 */
export function activateSyntheticBrowserReleaseFixture({ createdAt = new Date().toISOString(), files = {}, releaseId, repoRoot, }) {
    ...
    if (Boolean(currentEntry) !== Boolean(activationEntry)) {
        throw new Error("Synthetic browser release fixtures refuse to repair an incomplete active-release layout.");
    }
    if (currentEntry && !currentEntry.isSymbolicLink()) {
        throw new Error(`Synthetic active browser path must be a symbolic link: ${paths.currentBrowser}`);
    }
    if (activationEntry &&
        (!activationEntry.isFile() || activationEntry.isSymbolicLink())) {
        throw new Error(`Synthetic activation metadata must be a regular non-symlink file: ${paths.activation}`);
    }
    ...
    try {
        ...
        if (isNodeError(error) && error.code === "EEXIST") {
            throw new Error(`Synthetic browser release already exists and is immutable: ${releaseId}`, { cause: error });
        }
        throw error;
    }
    ...
    try {
        ...
        const indexSource = "<!doctype html><html><head>" +
        ...
        writeSyntheticFile(join(browserDir, SYNTHETIC_BUNDLE_FILENAME), "export {};\n");
        for (const fixtureFile of fixtureFiles) {
            const filePath = syntheticBrowserFilePath(browserDir, fixtureFile.relativePath);
        ...
        for (const relativePath of [
            ...
            stampSyntheticHtmlBuildId(syntheticBrowserFilePath(browserDir, relativePath), buildId);
        }
        ...
        if (sealed.artifactDigest !== unsealed.artifactDigest ||
            sealed.browserFiles !== unsealed.browserFiles ||
            sealed.htmlFiles !== unsealed.htmlFiles) {
            throw new Error(`Synthetic browser release changed while its build identity was stamped: ${releaseId}`);
        }
        ...
        const stagedRelease = listBrowserReleases(paths.repoRoot).find((release) => release.releaseId === releaseId);
        if (!stagedRelease) {
            throw new Error(`Synthetic browser release was rejected by the shared reader contract: ${releaseId}`);
        }
        const nonce = `${process.pid}-${randomUUID()}`;
        const temporaryPointer = join(paths.root, `.synthetic-current-${nonce}`);
        const temporaryActivation = join(paths.root, `.synthetic-activation-${nonce}.json`);
        try {
            symlinkSync(join("releases", releaseId, "browser"), temporaryPointer, "dir");
        ...
        if (active.releaseId !== releaseId) {
            throw new Error(`Synthetic browser release activation resolved an unexpected release: ${active.releaseId}`);
        }
...
function sealSyntheticBrowserRelease(root) {
    const directories = [];
    const pending = [root];
        ...
        try {
            for (let entry = handle.readSync(); entry; entry = handle.readSync()) {
                ...
                if (snapshot.isDirectory() && !snapshot.isSymbolicLink()) {
                    ...
                    throw new Error(`Synthetic browser release contains an unsupported entry: ${candidate}`);
                }
            }
        }
        finally {
...
function makeSyntheticReleaseWritable(root) {
    const entry = lstatIfPresent(root);
    if (!entry?.isDirectory() || entry.isSymbolicLink()) {
        throw new Error(`Synthetic browser release is not writable safely: ${root}`);
    }
...
function stampSyntheticHtmlBuildId(file, buildId) {
    let source;
    try {
...
export async function probeServer(origin, probes, fetchImplementation = globalThis.fetch) {
    ...
    if (!["http:", "https:"].includes(base.protocol) ||
        ...
        base.search ||
        base.hash) {
        throw new Error("Server probe origin must be an HTTP(S) origin without a path.");
    }
    const results = [];
    for (const probe of probes) {
        if (!probe.path.startsWith("/") || probe.path.startsWith("//")) {
            throw new Error(`Probe path must be a same-origin absolute path: ${probe.path}`);
        }
        const probeUrl = new URL(probe.path, base);
        if (probeUrl.origin !== base.origin) {
            throw new Error(`Probe path must resolve inside ${base.origin}: ${probe.path}`);
        }
        ...
        if (response.status !== probe.status) {
            throw new Error(`Expected ${probe.path} to return ${probe.status}; received ${response.status}.`);
        }
        results.push({ path: probe.path, status: response.status });
    }
    return results;
}
function probeHeaders(probe) {
...
function assertSyntheticReleaseId(releaseId) {
    if (typeof releaseId !== "string" || !SYNTHETIC_RELEASE_ID.test(releaseId)) {
        throw new Error(`Invalid synthetic browser release id: ${String(releaseId)}`);
    }
}
function normalizeSyntheticCreatedAt(createdAt) {
    if (typeof createdAt !== "string" ||
        !Number.isFinite(Date.parse(createdAt))) {
        throw new Error("Synthetic browser release createdAt must be an ISO timestamp.");
    }
    return new Date(createdAt).toISOString();
}
function normalizeSyntheticFiles(files) {
    if (!files || typeof files !== "object" || Array.isArray(files)) {
        throw new Error("Synthetic browser release files must be an object.");
    }
    const entries = Object.entries(files);
    if (entries.length > MAX_SYNTHETIC_FILES) {
        throw new Error(`Synthetic browser release exceeds the ${MAX_SYNTHETIC_FILES}-file limit.`);
    }
    const normalized = [];
    ...
    for (const [relativePath, content] of entries) {
        ...
        if (SYNTHETIC_BROWSER_FILES.has(portablePath)) {
            throw new Error(`Synthetic browser release file is owned by the fixture: ${relativePath}`);
        }
        if (portablePaths.has(portablePath)) {
            throw new Error(`Synthetic browser release file collides on portable filesystems: ${relativePath}`);
        }
        ...
        const bytes = typeof content === "string"
            ...
            : content instanceof Uint8Array
                ...
                : undefined;
        if (!bytes) {
            throw new Error(`Synthetic browser release file must be text or bytes: ${relativePath}`);
        }
        if (bytes.byteLength > MAX_SYNTHETIC_FILE_BYTES) {
            throw new Error(`Synthetic browser release file exceeds the ${MAX_SYNTHETIC_FILE_BYTES}-byte limit: ${relativePath}`);
        }
        totalBytes += bytes.byteLength;
        if (totalBytes > MAX_SYNTHETIC_TOTAL_BYTES) {
            throw new Error(`Synthetic browser release exceeds the ${MAX_SYNTHETIC_TOTAL_BYTES}-byte total limit.`);
        }
...
function assertSyntheticRelativePath(relativePath) {
    if (typeof relativePath !== "string" ||
        ...
        /[\u0000-\u001f\u007f\\]/.test(relativePath)) {
        throw new Error(`Synthetic browser release file must use a safe relative path: ${String(relativePath)}`);
    }
    const segments = relativePath.split("/");
    if (segments.some((segment) => !SYNTHETIC_FILE_SEGMENT.test(segment) ||
        segment === "." ||
        segment === "..")) {
        throw new Error(`Synthetic browser release file must use a safe relative path: ${relativePath}`);
    }
}
function requireSyntheticRepository(repoRoot) {
    const entry = lstatIfPresent(repoRoot);
    if (!entry?.isDirectory() || entry.isSymbolicLink()) {
        throw new Error(`Synthetic browser release repository must be an existing real directory: ${repoRoot}`);
    }
    return realpathSync(repoRoot);
}
function ensureSyntheticDirectory(directory, canonicalRepoRoot) {
    ...
    if (!entry.isDirectory() || entry.isSymbolicLink()) {
        throw new Error(`Synthetic browser release path must be a real directory: ${directory}`);
    }
    const canonical = realpathSync(directory);
    if (!isContainedPath(canonicalRepoRoot, canonical)) {
        throw new Error(`Synthetic browser release path escapes its repository: ${directory}`);
    }
}
function syntheticBrowserFilePath(browserDir, relativePath) {
    const candidate = resolve(browserDir, ...relativePath.split("/"));
    if (!isContainedPath(browserDir, candidate)) {
        throw new Error(`Synthetic browser release file escapes its browser root: ${relativePath}`);
    }
    return candidate;
}
function ensureSyntheticFileParent(browserDir, filePath) {
    ...
    for (const segment of relativeParent.split(sep)) {
        ...
        if (!entry.isDirectory() || entry.isSymbolicLink()) {
            throw new Error(`Synthetic browser release file parent must be a real directory: ${current}`);
        }
    }
}
function writeSyntheticFile(filePath, content) {
