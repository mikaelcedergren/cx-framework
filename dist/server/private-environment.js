...
import { ConfigurationError, nodeEnvironmentValue, } from "./configuration.js";
 ...
 */
export function privateEnvironmentFileStartupMode({ bypassKey, environment, }) {
    if (!/^[A-Z_][A-Z0-9_]*$/.test(bypassKey)) {
        throw new TypeError("Private environment bypassKey must be a canonical environment variable name.");
    }
    const nodeEnvironment = nodeEnvironmentValue(environment);
    const bypass = environment[bypassKey];
    if (bypass !== undefined && bypass !== "false") {
        throw new ConfigurationError(`${bypassKey} must be exactly false when it is set.`);
    }
    if (nodeEnvironment === "production") {
        if (bypass !== undefined) {
            throw new ConfigurationError(`${bypassKey}=false cannot disable the required production private environment file.`);
        }
        return "required-authoritative";
    }
    if (nodeEnvironment === "test" || bypass === "false")
        return "skip";
...
export class UnsupportedPrivateEnvironmentKeyError extends Error {
    ...
    constructor(file, key) {
        super(`Private environment file ${path.basename(file)} contains unsupported key ${key}.`);
 ...
 */
export function loadPrivateEnvironmentFile({ allowedKeys, environment, file, mode, }) {
    ...
    if (effectiveUserId === undefined ||
        noFollow === undefined ||
        nonBlock === undefined) {
        throw new Error("Private environment loading requires POSIX ownership, O_NOFOLLOW, and O_NONBLOCK.");
    }
    let descriptor;
    try {
        ...
        if (isErrno(error, "ENOENT") && mode === "optional-ambient")
            return false;
        if (isErrno(error, "ENOENT")) {
            throw new Error(`Required private environment file ${path.basename(file)} is absent.`, { cause: error });
        }
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Private environment file ${path.basename(file)} must be valid UTF-8.`, { cause: error });
        }
        ...
        for (const [name, value] of Object.entries(parsed)) {
            if (typeof value !== "string" ||
                name.includes("\0") ||
                value.includes("\0")) {
                throw new Error(`Private environment file ${path.basename(file)} must not contain NUL in parsed names or values.`);
            }
        }
...
function assertLoadOptions({ allowedKeys, environment, file, mode, }) {
    if (!path.isAbsolute(file)) {
        throw new TypeError("Private environment file must use an absolute path.");
    }
    if (!environment || typeof environment !== "object") {
        throw new TypeError("Private environment target must be an environment object.");
    }
    if (!(allowedKeys instanceof Set) || allowedKeys.size === 0) {
        throw new TypeError("Private environment allowedKeys must be a non-empty Set.");
    }
    if (mode !== "optional-ambient" && mode !== "required-authoritative") {
        throw new TypeError("Private environment mode must be optional-ambient or required-authoritative.");
    }
    for (const name of allowedKeys) {
        if (!/^[A-Z_][A-Z0-9_]*$/.test(name)) {
            throw new TypeError("Private environment allowedKeys must contain canonical environment variable names.");
        }
    }
}
function assertPrivateEnvironmentFile(metadata, file, effectiveUserId) {
    if (!metadata.isFile() ||
        metadata.isSymbolicLink() ||
        metadata.nlink !== 1n ||
        metadata.uid !== BigInt(effectiveUserId) ||
        (metadata.mode & 511n) !== BigInt(PRIVATE_FILE_MODE)) {
        throw privateFileError(file);
    }
}
function assertPrivateEnvironmentFileSize(size, file) {
...
function privateFileError(file, cause) {
    return new Error(`Private environment file ${path.basename(file)} must be one owner-owned mode-0600 regular file with one link and no final symlink.`, cause === undefined ? undefined : { cause });
}
function privateEnvironmentFileTooLarge(file) {
    return new Error(`Private environment file ${path.basename(file)} exceeds 64 KiB.`);
}
function privateEnvironmentFileChanged(file, cause) {
    return new Error(`Private environment file ${path.basename(file)} changed while loading.`, cause === undefined ? undefined : { cause });
}
