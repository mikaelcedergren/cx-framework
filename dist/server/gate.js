...
const DEFAULT_GATE_CONTENT_SECURITY_POLICY = "default-src 'none'; style-src 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'";
const PRESENTED_GATE_CONTENT_SECURITY_POLICY = "default-src 'none'; style-src 'self'; img-src 'self'; font-src 'self'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'";
const PRESENTATION_FORBIDDEN_ELEMENT = /<\s*\/?\s*(?:applet|audio|base|button|caption|col|colgroup|datalist|embed|fencedframe|form|frame|frameset|iframe|input|math|noembed|noframes|noscript|object|plaintext|portal|script|select|style|svg|table|tbody|td|template|textarea|tfoot|th|thead|tr|video|xmp)(?:\s|\/|>)/iu;
const PRESENTATION_FORBIDDEN_BODY_ELEMENT = /<\s*\/?\s*title(?:\s|\/|>)/iu;
const PRESENTATION_FORBIDDEN_ATTRIBUTE = /(?:\s|\/)(?:action|form(?:action|enctype|method|novalidate|target)?|http-equiv|imagesrcset|manifest|on[a-z0-9_.:-]+|ping|srcdoc|srcset|style)\s*=/iu;
const PRESENTATION_URL_ATTRIBUTE = /(?:^|[\s</])(?:cite|href|poster|src)\s*=/giu;
const PRESENTATION_QUOTED_URL_ATTRIBUTE = /(?:^|[\s</])(?:cite|href|poster|src)\s*=\s*(["'])([^"']*)\1/giu;
...
function validatePresentationTemplate(template) {
    ...
    if (slotIndex < 0 ||
        ...
        !documentOrder.every((index, position) => position === 0 || index > documentOrder[position - 1]) ||
        ...
        templateWithoutSlot.includes("-->")) {
        throw new Error("A site gate presentation template must contain one ordered html/head/body document and one body-level framework form slot.");
    }
    if (PRESENTATION_FORBIDDEN_ELEMENT.test(templateWithoutSlot) ||
        PRESENTATION_FORBIDDEN_BODY_ELEMENT.test(template.slice(documentOrder[3], documentOrder[4])) ||
        PRESENTATION_FORBIDDEN_ATTRIBUTE.test(templateWithoutSlot)) {
        throw new Error("A site gate presentation template cannot contain executable, embedded, inline-style, or form-owned markup.");
    }
    const declarations = [
    ...
    if (declarations.length !== quoted.length) {
        throw new Error("Site gate presentation URL attributes must be quoted same-origin paths.");
    }
    for (const match of quoted) {
        ...
        if ((!value.startsWith("#") &&
            ...
            /[&\\\u0000-\u0020\u007f<>]/.test(value)) {
            throw new Error("Site gate presentation URL attributes must use same-origin absolute paths or fragments.");
        }
    }
}
function hasWellDelimitedPresentationMarkup(template, slotIndex) {
    let insideTag = false;
    let quote;
