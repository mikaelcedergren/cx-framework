...
RULE-ID: dialogs.dismissal-request SCOPE: component COMPONENT: dialogs TYPE: MUST TOPIC: interaction RULE: Resolve a cancellable user exit before closing begins. DESCRIPTION: Cancel and dismiss paths, including Escape when it maps to the same visible action, give the owning product a synchronous chance to keep the dialog's rendered surface, state, open state, and overlay registration intact and run no closing focus restoration or fallback. A nested confirmation may then take top interaction and focus ownership. Blocking the exit produces no dismissal outcome or close lifecycle; completion, programmatic closure, and destruction bypass this request.

RULE-ID: dialogs.focus SCOPE: component COMPONENT: dialogs TYPE: MUST TOPIC: accessibility RULE: Move focus into an opened modal and restore it deliberately on close. DESCRIPTION: Modal focus remains within the topmost active dialog while it is open. After child teardown and parent focus maintenance settle, return focus to the exact invoking control when it remains connected, visible, and focusable; parent containment or initial-focus routines may not overwrite it, though a newly opened top overlay takes precedence. Otherwise move focus to the established deterministic target inside the owning surviving parent surface, never the document body, removed content, or behind another active modal.
...
RULE-ID: detail-panels.dismissal-request SCOPE: component COMPONENT: detail-panels TYPE: MUST TOPIC: interaction RULE: Resolve a cancellable user exit before panel dismissal begins. DESCRIPTION: Close-button, Escape, and enabled outside-click paths give the owning product a synchronous chance to keep the panel's rendered surface, state, and overlay registration intact and run no closing focus restoration or fallback. A nested confirmation may then take top interaction and focus ownership. Blocking the exit produces no dismissal outcome or close lifecycle; programmatic removal and destruction bypass this request.
