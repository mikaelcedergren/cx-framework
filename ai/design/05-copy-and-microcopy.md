...
RULE-ID: copy.dates.relative SCOPE: relative-time TYPE: MUST TOPIC: data-display RULE: Use relative labels for activity, updates, and scheduled-date summaries within seven days of now; use an absolute date outside that window. DESCRIPTION: Activity may say `Just now`, `12m ago`, or `4h ago`; calendar summaries may say `Today`, `Yesterday`, `Tomorrow`, `3d ago`, or `In 3d`, optionally followed by 24-hour `HH:mm`. Use one shared formatter and preserve access to an exact date wherever precision matters. EXCEPT: Date inputs, calendar grids, chart axes, transaction and audit records, exports, and other precision-dependent contexts retain absolute dates.

RULE-ID: copy.dates.calendar SCOPE: relative-time TYPE: MUST TOPIC: data-display RULE: Interpret Today, Yesterday, and Tomorrow as calendar days in the product's display timezone, not elapsed 24-hour periods. DESCRIPTION: Keep date-only values as calendar dates without inventing a time; handle past and future values symmetrically, show an explicit missing value for unknown dates, and refresh relative labels as time passes.
